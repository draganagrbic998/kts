...
public class CategoryServiceTest {
	...
	public void testHasNameNewCategoryExisting() {
		...
		Mockito.when(this.categoryRepository.findByName(param.getName())).thenReturn(new Category());
	...
	public void testHasNameNewCategoryNonExisting() {
		...
		Mockito.when(this.categoryRepository.findByName(param.getName())).thenReturn(null);
		assertFalse(this.categoryService.hasName(param));
	}
	
	@Test
	public void testFilterNamesEmpty() {
		Mockito.when(this.categoryRepository.filterNames(FilterConstants.FILTER_ALL))
		.thenReturn(List.of(CategoryConstants.NAME_ONE, CategoryConstants.NAME_THREE, CategoryConstants.NAME_TWO));
		List<String> names = 
	...
	public void testFilterNamesAll() {
		Mockito.when(this.categoryRepository.filterNames(CategoryConstants.FILTER_NAME_ALL))
		.thenReturn(List.of(CategoryConstants.NAME_ONE, CategoryConstants.NAME_THREE, CategoryConstants.NAME_TWO));
		List<String> names = 
				this.categoryService
	...
	public void testFilterNamesOne() {
		Mockito.when(this.categoryRepository.filterNames(FilterConstants.FILTER_ONE))
		.thenReturn(List.of(CategoryConstants.NAME_ONE));
		List<String> names = 
				this.categoryService
	...
	public void testListAll() {
		Mockito.when(this.categoryRepository.findAll(this.pageableAll))
		.thenReturn(new PageImpl<>(List.of(this.listCategory(1), this.listCategory(2), this.listCategory(3))));
	...
	public void testListAllPaginated() {
		Mockito.when(this.categoryRepository.findAll(this.pageablePart))
		.thenReturn(new PageImpl<>(List.of(this.listCategory(1), this.listCategory(2))));
		List<Category> categories = 
				this.categoryService
				.list(this.pageablePart).getContent();
		assertEquals(MainConstants.TOTAL_SIZE - 1, categories.size());
		assertEquals(CategoryConstants.ID_ONE, categories.get(0).getId());
		assertEquals(CategoryConstants.NAME_ONE, categories.get(0).getName());
		assertEquals(CategoryConstants.ID_TWO, categories.get(1).getId());
		assertEquals(CategoryConstants.NAME_TWO, categories.get(1).getName());
	...
	public void testListAllNonExistingPage() {
		Mockito.when(this.categoryRepository.findAll(this.pageableNonExisting))
	...
	public void testDeleteExistingWithCulturalOffer() {
		Mockito.doThrow(ConstraintViolationException.class).when(this.categoryRepository).deleteById(CategoryConstants.ID_THREE);
		this.categoryService.delete(CategoryConstants.ID_THREE);
	}
	
	@Test
	public void testDeleteExistingWithoutCulturalOffer() {
		Mockito.when(this.categoryRepository.count()).thenReturn((long) MainConstants.TOTAL_SIZE);
		long size = this.categoryRepository.count();
		Category category = this.testingCategory();
		Mockito.doReturn(category).when(this.categoryRepository).save(category);
		this.categoryService.save(category);
		Mockito.when(this.categoryRepository.count()).thenReturn(size + 1);
		assertEquals(size + 1, this.categoryRepository.count());
		this.categoryService.delete((long) MainConstants.TOTAL_SIZE + 1);
		Mockito.when(this.categoryRepository.count()).thenReturn((long) MainConstants.TOTAL_SIZE);
	...
	public void testDeleteNonExisting() {
		Mockito.doThrow(EmptyResultDataAccessException.class).when(this.categoryRepository).deleteById(MainConstants.NON_EXISTING_ID);
	...
	public void testAddNullName() {
		...
		Mockito.when(this.categoryRepository.save(category)).thenThrow(ConstraintViolationException.class);
		this.categoryService.save(category);
	}
	...
	public void testAddNonUniqueName() {
		Category category = this.testingCategory();
		category.setName(CategoryConstants.NAME_ONE);
		Mockito.when(this.categoryRepository.save(category)).thenThrow(DataIntegrityViolationException.class);
		this.categoryService.save(category);
	}
