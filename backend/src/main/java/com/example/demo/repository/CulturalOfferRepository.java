...
public interface CulturalOfferRepository extends JpaRepository<CulturalOffer, Long> {
	...
	@Query("select co.name from CulturalOffer co where lower(co.name) like lower(concat('%', :filter, '%'))")
	public List<String> filterNames(String filter);
	
	@Query("select co.location from CulturalOffer co where lower(co.location) like lower(concat('%', :filter, '%'))")
	public List<String> filterLocations(String filter);
	
	@Query("select co.type.name from CulturalOffer co where lower(co.type.name) like lower(concat('%', :filter, '%'))")
